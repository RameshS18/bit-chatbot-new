...
# --- MODIFIED: Import Google Embeddings ---
...
# --- MODIFIED: Removed the old BGE model path ---
# --- MODIFIED: New path for the Google-powered FAISS index ---
...
def get_or_create_faiss_index():
    ...
    creates embeddings (using the Google API), builds a FAISS index,
    and saves it to disk.
    """
    # FAISS_INDEX_PATH is now "faiss_index_google"
    if os.path.exists(FAISS_INDEX_PATH):
        ...
        try:
            ...
            vectorstore = FAISS.load_local(
                FAISS_INDEX_PATH,
                embedding_model,  # This is now the Google Embedding model object
    ...
    print("Creating FAISS vector store... (This will call the Google API and may take time)")
    # This step will now call the Google API to get embeddings for all chunks.
    # This will use 0 of your RAM, but requires internet and time.
    vectorstore = FAISS.from_documents(
        documents=texts,
        embedding=embedding_model # This is now the Google Embedding model object
    )
...
def login_user():
    ...
    try:
        ...
        # --- Your Existing Login/Registration Logic (This part is untouched) ---
        timestamp = datetime.now(IST_TZ).isoformat()
        conn = sqlite3.connect(USERS_DB_PATH)
        cursor = conn.cursor()
        ...
        if 'conn_otp' in locals() and conn_otp:
            conn_otp.close()
...
def chat():
    try:
        ...
        # This will now use the Google API to embed the query
